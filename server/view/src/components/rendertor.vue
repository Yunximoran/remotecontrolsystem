...
export default {
    ...
    methods: {
        loadoptions(node, label, options, level) {
            ...
            this.is_show_options[level] = !is_unfold;
        ...
        alter(current, option) {
            ...
            const otherAlters = current.querySelectorAll('.alter');
            if (otherAlters.length === 0) {
                render(inp, current);
            } else {
                render(null, inp.parentElement());
            }
            // 这里监听 alter 方法的调用
            this.listenToAlterCalls(current, option);
        },

        listenToAlterCalls(current, option) {
            console.log(`alter 方法被调用，option: ${option}`);
            // 你可以在这里添加更多的逻辑来处理 alter 方法的调用
        }
    },
